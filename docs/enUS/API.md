...
Request bodies larger than `HTTP_BODY_LIMIT_BYTES` are rejected by the HTTP server with status `413` before JSON parsing. Fiber-level errors, including body-limit, missing-route, and unsupported-method failures, use the same JSON response shape as `/v1/send`.
